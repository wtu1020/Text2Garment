...
class UNet2DConditionOutput(BaseOutput):
    ...
    anc_features: Tuple[torch.FloatTensor] = None
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        if isinstance(only_cross_attention, bool):
            if mid_block_only_cross_attention is None: 
                mid_block_only_cross_attention = only_cross_attention # false 
        ...
        if isinstance(num_attention_heads, int): # 8
        ...
        if class_embeddings_concat:
            ...
            blocks_time_embed_dim = time_embed_dim # 1280 
    ...
    def forward(
        ...
        camera: Optional[torch.Tensor] = None, # R matrix flattened to 9 channels
        ...
        # camera_emb
        camera = camera.expand(sample.shape[0], -1) 
        camera_emb = self.camera_embedding(camera) if camera is not None else 0 
        ...
        ## mix first down_block results of master and branch
        sample_list = [sample]
        for i in range(self.branch_num):
    ...
    def from_pretrained_2d(
        ...
        # use attn1 and norm1 (pretrained self attn) to initialize attn0 and norm0 (single/multi-view 3D attn)
        extra_state_dict = {} 
        for k in state_dict:
        ...
        for name, param in model.named_parameters():
            if 'branch' in name and 'camera' not in name:
                name_splits = name.split('.')
                master_name = '.'.join([name_splits[0]]+name_splits[2:]).replace("_branch", "") # remove branch num and "branch""
                                                                                                # e.g. 'up_blocks_branch.0.0.attentions.0.norm.bias' -> 'up_blocks.0.attentions.0.norm.bias'
                if 'up_blocks' in name: 
                    # [0,1,2,3]
                    # [      0] if copy 1 block
                    # [    0,1] if copy 2 block
                    # [  0,1,2] if copy 3 block
                    # [0,1,2,3] if copy 4 block
                    ...
                    master_name = '.'.join([name_splits[0], str(block_id_in_master)]+name_splits[2:]) # e.g. 'up_blocks.0.attentions.0.norm.bias' -> 'up_blocks.3.attentions.0.norm.bias'
        ...
        m, u = model.load_state_dict(state_dict, strict=False) # not strict due to missing camera weights
